...
def dijkstra_with_time(locations, demands, distance_matrix, time_windows, vehicle_capacity):
    depot = "Depósito"
    ...
    while clients:
        pq = []  # Fila de prioridade para encontrar o cliente mais próximo
        for client in clients:
            if demands[client] <= current_capacity:
                ...
                if arrival_time <= end_window:
                    heapq.heappush(pq, (dist, client))

        if not pq:
            # Retornar ao depósito se não houver clientes atendíveis
            dist, travel_time = distance_matrix.get((route[-1], depot), (0, 0))
        ...
        dist, next_client = heapq.heappop(pq)
    ...
    return route, total_distance, total_time, route.count(depot) - 1

# Resolver o problema
final_route, total_distance, total_time, visits_to_depot = dijkstra_with_time(locations, demands, distance_matrix, time_windows, vehicle_capacity)
