...
def savings_algorithm_with_time(locations, demands, distance_matrix, time_windows, vehicle_capacity):
    depot = "Depósito"
    ...
    while clients:
        ...
        for client in clients:
            if demands[client] <= current_capacity:
                _, travel_time = distance_matrix.get((route[-1], client), (0, 0))
                ...
                if arrival_time <= end_window and arrival_time < best_time:
...
# Resolver o problema
final_route, total_distance, total_time, visits_to_depot = savings_algorithm_with_time(locations, demands, distance_matrix, time_windows, vehicle_capacity)
