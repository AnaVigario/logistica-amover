...
const CalendarPage: React.FC = () => {
  ...
  const renderDay = (day: number) => {
    ...
    return (
      <div
        ...
        className="h-20 border border-gray-200 p-1 relative cursor-pointer hover:bg-gray-50"
      >
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* GRID */}
      <div className="p-3 grid grid-cols-7 text-center font-semibold text-sm text-gray-600">
        {["D", "S", "T", "Q", "Q", "S", "S"].map((d) => (
          <div key={d}>{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 flex-1">
        {[...Array(firstDayOfMonth)].map((_, i) => (
          <div key={i} className="h-20 border border-gray-200 bg-gray-50" />
        ))}
