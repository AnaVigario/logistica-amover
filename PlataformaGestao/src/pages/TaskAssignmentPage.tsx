      ...
      <div className="w-1/3 bg-white border-r p-4 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          {sortTasksSmart(filteredTasks).map(task => {
            ...
            return (
              <div key={task.id}
                className={`border rounded-lg p-3 cursor-pointer hover:shadow ${assigned ? "bg-green-50 border-green-300" : "border-gray-200"}`}
...
>
  ...
  list.map((t, i) => (
    <div
      key={t.id}
      className="bg-white border rounded-lg p-3 mt-3 shadow-sm"
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => removeTask(moto.id, t.id)}>
            <Trash2 size={18} className="text-red-500 hover:text-red-700" />
                ...
                <p className="mt-2 text-sm text-gray-500 border rounded p-2 text-center">
      ...
      {showAssignModal && selectedTask && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center">
          <div className="bg-white p-4 rounded w-80">
           ...
           <button
            key={vehicle.id}
            className="w-full border p-2 mb-2 rounded hover:bg-gray-100 text-left"
